...
class DatabaseTests(unittest.TestCase):
    ...
    def test_ralph_loop_run_rejects_malformed_required_evidence(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            conn = self.open_db(tmpdir)
            task_id = worker_db.create_task(conn, name="qa-task", goal="Run QA.")

            with self.assertRaisesRegex(WorkerError, "required_before_continue must be a list"):
                worker_db.create_ralph_loop_run(
                    conn,
                    task_id=task_id,
                    max_iterations=2,
                    required_before_continue="ci_green",  # type: ignore[arg-type]
                )
    ...
    def test_enqueue_helpers_reject_unknown_required_permission(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            conn = self.open_db(tmpdir)
            self.insert_task(conn, "task-enqueue", "enqueue-task")

            with self.assertRaises(ValueError):
                worker_db.enqueue_notify_manager(
                    conn,
                    task_id="task-enqueue",
                    message="inspect worker",
                    required_permission="communication.not_real",
                )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaises(WorkerError):
                worker_db.finish_command_attempt(
                    ...
                    state="failed",
                    error="late failure",
                    timestamp="2026-05-23T10:01:02Z",
                )
            ...
            telemetry = [
                ...
                if event["event_type"] in {"dispatch_command_succeeded", "dispatch_command_failed"}
            ]
            self.assertEqual(command_row["state"], "succeeded")
            self.assertIsNone(command_row["error"])
            self.assertEqual(attempt_row["state"], "succeeded")
            self.assertIsNone(attempt_row["error"])
            self.assertEqual([event["event_type"] for event in telemetry], ["dispatch_command_succeeded"])
    ...
    def test_manager_config_round_trips(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(worker_db.manager_config(conn, task_id=task_id)["revision"], 1)
...
class DispatchTests(unittest.TestCase):
    ...
    def test_dispatch_completion_notification_has_correlation_chain(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            notifications = worker_db.routed_notifications(conn, task_id="task-dispatch")
            worker_db.consume_routed_notifications_for_cycle(
                conn,
                task_id="task-dispatch",
                binding_id=notifications[0]["binding_id"],
    ...
    def test_dispatch_completion_correlation_uses_consumed_cycle_not_next_cycle_heuristic(self):
        from workerctl import supervise_cycle

        with tempfile.TemporaryDirectory() as tmpdir:
            conn, db_path = self.open_db(tmpdir)
            worker_id, manager_id = self.setup_bound_task(conn)
            rollout = Path(tmpdir) / "worker.jsonl"
            rollout.write_text(json.dumps({"type": "session_meta", "payload": {"id": "codex-worker"}}) + "\n")
            conn.execute(
                "update sessions set codex_session_path = ? where id = ?",
                (str(rollout), worker_id),
            )
            binding_id = conn.execute(
                "select id from bindings where task_id = 'task-dispatch'"
            ).fetchone()["id"]
            event_id = self.insert_completion(conn, worker_id, timestamp="2026-05-23T10:01:00Z")
            notification_id = worker_db.insert_routed_notification(
                conn,
                task_id="task-dispatch",
                binding_id=binding_id,
                correlation_id="dispatch-consume",
            ...
            decoy_cycle_id = worker_db.create_manager_cycle(
                conn,
                task_id="task-dispatch",
                manager_id=None,
                timestamp="2026-05-23T10:02:00Z",
            ...
            worker_db.finish_manager_cycle(
                conn,
                cycle_id=decoy_cycle_id,
                state="succeeded",
                status={"kind": "session_cycle", "task": "dispatch-task"},
                timestamp="2026-05-23T10:02:01Z",
            )
            worker_db.insert_manager_decision(
                conn,
                task_id="task-dispatch",
                manager_id=None,
                manager_cycle_id=decoy_cycle_id,
                decision="inspect",
                reason="decoy cycle should not consume notification",
            )
            conn.commit()

            with mock.patch.object(supervise_cycle, "_pid_is_alive", return_value=True):
            ...
            chain = next(
                chain
                for chain in audit["correlation_chains"]
                if chain["routed_notification_ids"] == [notification_id]
            )
    ...
    def test_dispatch_concurrent_completion_race_uses_real_dedupe_constraint(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            conn, db_path = self.open_db(tmpdir)
            worker_id, _manager_id = self.setup_bound_task(conn)
            self.insert_completion(conn, worker_id)
            ...
            original_insert = worker_db.insert_routed_notification

            def racing_insert(*args, **kwargs):
                if kwargs.get("signal_type") == "worker_task_complete":
            ...
            def run_dispatcher(dispatcher_id: str) -> None:
                args = argparse.Namespace(
                    type=None,
                )
                try:
                    batch = commands._dispatch_once_pass(
                        args,
                        worker_db=worker_db,
            ...
            with mock.patch.object(worker_db, "insert_routed_notification", new=racing_insert):
                with mock.patch.object(worker_tmux, "send_text_to_session", side_effect=fake_send):
                    threads = [
                        threading.Thread(target=run_dispatcher, args=("dispatch-a",)),
                        threading.Thread(target=run_dispatcher, args=("dispatch-b",)),
                    ]
            ...
            processed = [item for output in outputs for item in output]
            self.assertEqual(sorted(item["state"] for item in processed), ["delivered", "suppressed"])
            self.assertEqual(len(worker_db.routed_notifications(conn, task_id="task-dispatch")), 1)
    ...
    def test_dispatch_keeps_pre_send_completion_failure_recoverable(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def fail_before_send(conn, *, session_name, text, dry_run=False, side_effect_audit=None, side_effect_started_callback=None):
    ...
    def test_dispatch_leases_completion_notification_before_send(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            conn, db_path = self.open_db(tmpdir)
            worker_id, _manager_id = self.setup_bound_task(conn)
            self.insert_completion(conn, worker_id)
            ...
            def fake_send(
                ...
                *,
                ...
                text,
                dry_run=False,
                side_effect_audit=None,
                side_effect_started_callback=None,
            ):
                notification = worker_db.routed_notifications(conn, task_id="task-dispatch")[0]
                observed["claimed_by"] = notification["claimed_by"]
                observed["claim_expires_at"] = notification["claim_expires_at"]
                observed["side_effect_started"] = notification["side_effect_started"]
                return {"target": "tmux-manager:0.0"}
            ...
            self.assertEqual(observed["claimed_by"], "dispatch-test")
            self.assertIsNotNone(observed["claim_expires_at"])
            self.assertFalse(observed["side_effect_started"])

    def test_dispatch_marks_completion_started_before_paste_failure(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            conn, db_path = self.open_db(tmpdir)
            worker_id, _manager_id = self.setup_bound_task(conn)
            self.insert_completion(conn, worker_id)
            ...
            def fake_send(
                ...
                *,
                ...
                self.assertIsNotNone(side_effect_started_callback)
                ...
                raise RuntimeError("tmux paste failed after start")
            ...
            self.assertTrue(notification["side_effect_started"])
            self.assertFalse(notification["side_effect_completed"])
            self.assertIn("tmux paste failed after start", notification["error"])
    ...
    def test_dispatch_blocks_continue_iteration_missing_ci_green_before_no_tmux_worker_inbox(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            decision_id = worker_db.insert_manager_decision(
                ...
                manager_id=None,
                decision="nudge",
                reason="Request iteration 2 before CI evidence exists.",
                payload={"ralph_loop_run_id": loop_run_id, "requested_iteration": 2},
            )
            command_id = worker_db.enqueue_continue_iteration(
                conn,
            ...
            self.assertEqual(processed["state"], "blocked")
            self.assertEqual(processed["reason"], "missing_ci_green_evidence")
            self.assertEqual(processed["missing_evidence"], ["ci_green"])
            self.assertFalse(processed["delivered"])
            self.assertFalse(processed["target_worker_notified"])
            self.assertEqual(processed["current_iteration"], 1)
            ...
            attempt_result = json.loads(attempt_row["result_json"])
            self.assertEqual(attempt_result["state"], "blocked")
            self.assertEqual(attempt_result["missing_evidence"], ["ci_green"])
            self.assertFalse(attempt_result["target_worker_notified"])
            self.assertEqual(attempt_row["side_effect_started"], 0)
            self.assertEqual(attempt_row["side_effect_completed"], 0)
            self.assertEqual(worker_db.routed_notifications(conn, task_id="task-dispatch"), [])
    ...
    def test_dispatch_blocks_continue_iteration_missing_ci_green_before_tmux_send(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            conn, db_path = self.open_db(tmpdir)
            self.setup_bound_task(conn)
            loop_run_id = worker_db.create_ralph_loop_run(
            ...
            worker_db.enqueue_continue_iteration(
                ...
                loop_run_id=loop_run_id,
                requested_iteration=2,
                correlation_id="ralph-loop-missing-ci-tmux",
            )
            conn.commit()
    ...
    def test_dispatch_allows_continue_iteration_after_ci_green_evidence_to_no_tmux_worker_inbox(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            conn, db_path = self.open_db(tmpdir)
            worker_id, _manager_id = self.setup_bound_task(conn)
            conn.execute("update sessions set tmux_session = null where id = ?", (worker_id,))
            ...
            worker_db.insert_acceptance_criterion(
                ...
                evidence={
                    "correlation_id": "ralph-loop-ci-green",
            ...
            self.assertEqual(notification["payload"]["ralph_loop"]["required_before_continue"], ["ci_green"])
    ...
    def test_replay_exposes_missing_evidence_continue_iteration_block(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            conn, db_path = self.open_db(tmpdir)
            worker_id, _manager_id = self.setup_bound_task(conn)
            conn.execute("update sessions set tmux_session = null where id = ?", (worker_id,))
            ...
            command_id = worker_db.enqueue_continue_iteration(
                ...
                loop_run_id=loop_run_id,
                requested_iteration=2,
                correlation_id="ralph-loop-replay-missing-ci",
            )
            conn.commit()
            ...
            self.assertEqual(len(attempts), 1)
            self.assertEqual(attempts[0]["result"]["state"], "blocked")
            self.assertEqual(attempts[0]["result"]["reason"], "missing_ci_green_evidence")
            self.assertEqual(attempts[0]["result"]["missing_evidence"], ["ci_green"])
            self.assertEqual(audit["routed_notifications"], [])

            entries = replay.replay_entries(audit)
            attempt_entry = next(entry for entry in entries if entry["source"] == "command_attempts")
            self.assertEqual(attempt_entry["details"]["result"]["state"], "blocked")
            self.assertEqual(attempt_entry["details"]["result"]["reason"], "missing_ci_green_evidence")
            self.assertEqual(attempt_entry["details"]["result"]["missing_evidence"], ["ci_green"])
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def fake_send(conn, *, session_name, text, dry_run=False, side_effect_audit=None, side_effect_started_callback=None):
                if side_effect_audit is not None:
                    side_effect_audit["side_effect_started"] = True
                    side_effect_audit["side_effect_completed"] = True
                with worker_db.connect(db_path) as other:
                    worker_db.initialize_database(other)
                    command_row = other.execute(
                        "select state from commands where id = ?",
                        (command_id,),
                    ).fetchone()
                    self.assertEqual(command_row["state"], "attempted")
                    worker_db.create_command(
                        other,
                        command_type="notify_manager",
                        task_id="task-dispatch",
                        payload={"message": "other writer is not blocked"},
                    )
                    other.commit()
                return {"target": "tmux-manager:0.0"}
            ...
            other_count = conn.execute(
                "select count(*) as count from commands where payload_json like '%other writer%'"
            ).fetchone()["count"]
            self.assertEqual(command_row["state"], "succeeded")
    ...
    def test_replay_exposes_dispatch_correlation_chain(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            notification_id = worker_db.routed_notifications(conn, task_id="task-dispatch")[0]["id"]
            conn.execute(
                """
                insert into manager_cycles(task_id, started_at, completed_at, state, status_json)
                values ('task-dispatch', '2026-05-23T10:02:00Z', '2026-05-23T10:02:01Z', 'succeeded', '{}')
            ...
            consumed_cycle_id = conn.execute(
                "select max(id) as id from manager_cycles"
            ).fetchone()["id"]
            worker_db.consume_routed_notifications_for_cycle(
                conn,
                task_id="task-dispatch",
                binding_id=conn.execute("select id from bindings where task_id = 'task-dispatch'").fetchone()["id"],
                manager_cycle_id=consumed_cycle_id,
                timestamp="2026-05-23T10:02:00Z",
            )
            conn.commit()
            ...
            self.assertEqual(chain["manager_decision_id"], decision_id)
            self.assertEqual(chain["manager_decision_cycle_id"], cycle_id)
            self.assertEqual(chain["manager_cycle_id"], consumed_cycle_id)
            self.assertEqual(len(chain["attempt_ids"]), 1)
            self.assertEqual(len(chain["routed_notification_ids"]), 1)
            self.assertEqual(chain["routed_notification_ids"], [notification_id])
            entries = replay.replay_entries(audit)
...
class CliTests(unittest.TestCase):
    ...
    def test_dashboard_ensure_dispatch_reuses_active_dispatch_heartbeat(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            db_path = Path(tmpdir) / "workerctl.db"
            with worker_db.connect(db_path) as conn:
                worker_db.initialize_database(conn)
    ...
    def test_enqueue_dispatch_commands_cli_records_required_permission(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(payload["correlation_id"].startswith("dispatch-"))
            with worker_db.connect(db_path) as conn:
                command = conn.execute(
                    """
                    select task_id, type, required_permission, correlation_id, payload_json
                    from commands
                    where id = ?
                    """,
                    (payload["command_id"],),
    ...
    def test_runs_cli_creates_ralph_loop_policy_when_active_telemetry_run_exists(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            proc = self.run_workerctl(
                ...
                "ralph_loop",
                "--metadata-json",
                '{"kind":"ralph_loop","max_iterations":1,"current_iteration":1,"cleanup_policy":"clear","required_before_continue":["ci_green"],"stop_conditions":["max_iterations"]}',
                "--path",
                str(db_path),
            )
            ...
            self.assertEqual(payload["metadata"]["required_before_continue"], ["ci_green"])
            with worker_db.connect(db_path) as conn:
                worker_db.initialize_database(conn)
                self.assertEqual(worker_db.active_run_for_task(conn, task_id=task_id)["id"], active_run_id)
                loop_run = worker_db.ralph_loop_run(conn, run=payload["id"])
                self.assertEqual(loop_run["current_iteration"], 1)
                self.assertEqual(loop_run["required_before_continue"], ["ci_green"])
    ...
    def test_runs_cli_rejects_malformed_required_evidence_cleanly(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            db_path = Path(tmpdir) / "workerctl.db"
            with worker_db.connect(db_path) as conn:
                worker_db.initialize_database(conn)
            ...
            proc = self.run_workerctl(
                ...
                "ralph_loop",
                "--metadata-json",
                '{"kind":"ralph_loop","max_iterations":2,"required_before_continue":"ci_green"}',
                "--path",
                str(db_path),
            )

            self.assertEqual(proc.returncode, 1)
            self.assertIn("ralph_loop run metadata required_before_continue must be a JSON array", proc.stderr)
            self.assertNotIn("Traceback", proc.stderr)
...
class SuperviseCycleTests(unittest.TestCase):
    ...
    def test_run_cycle_requires_acknowledgements_when_manager_configured(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("cycle requires current acknowledgement", str(raised.exception))
            worker_db.insert_task_acknowledgement(
                conn,
                task_id=task["id"],
                role="worker",
    ...
    def test_run_cycle_rejects_stale_acknowledgements_after_manager_config_revision_changes(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaises(WorkerError) as raised:
                supervise_cycle.run_cycle(conn, task_name="t", now="2026-05-11T14:32:17Z")

            self.assertIn("current acknowledgement", str(raised.exception))
...
class PairCommandTests(unittest.TestCase):
    ...
    def _bind_continuation_sessions(self, conn, *, task_name, tmpdir):
        ...
        manager_rollout = root / f"{task_name}-manager-rollout.jsonl"
        worker_rollout.write_text("WORKER_ROLLOUT_SECRET", encoding="utf-8")
        manager_rollout.write_text("MANAGER_ROLLOUT_SECRET", encoding="utf-8")
        worker_db.register_session(
            conn,
            name=f"{task_name}-worker-session",
            role="worker",
            codex_session_path=str(worker_rollout),
            codex_session_id=f"{task_name}-codex-worker",
            pid=111,
            cwd=tmpdir,
        )
        worker_db.register_session(
            conn,
            name=f"{task_name}-manager-session",
            role="manager",
            codex_session_path=str(manager_rollout),
            codex_session_id=f"{task_name}-codex-manager",
            pid=112,
            cwd=tmpdir,
        )
        worker_db.bind_sessions(
            conn,
            task_name=task_name,
            worker_session_name=f"{task_name}-worker-session",
            manager_session_name=f"{task_name}-manager-session",
        )
    ...
    def test_continuation_reviewer_runner_records_isolated_review(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self._bind_continuation_sessions(conn, task_name="continuation-runner-task", tmpdir=tmpdir)
                conn.commit()
            finally:
                conn.close()
    ...
    def test_continuation_reviewer_runner_failure_records_stop(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self._bind_continuation_sessions(conn, task_name="continuation-runner-fail-task", tmpdir=tmpdir)
                conn.commit()
            finally:
                conn.close()
    ...
    def test_continuation_reviewer_runner_sandbox_denies_bound_artifacts(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            conn = worker_db.connect(db_path)
            try:
                task_id = worker_db.create_task(conn, name="continuation-sandbox-task", goal="Review next step.")
                worker_db.upsert_manager_config(
                    conn,
                    task_id=task_id,
                    supervision_mode="guided",
            ...
            self.assertNotIn("MANAGER_ROLLOUT_SECRET", output)
            self.assertNotIn("WORKER_ROLLOUT_SECRET", output)
            review = json.loads(proc.stdout)
            self.assertEqual(review["agreement"], "divergent")
            self.assertEqual(review["verdict"], "stop")
