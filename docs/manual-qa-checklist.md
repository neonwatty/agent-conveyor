...
- [ ] `conveyor qa-run setup-bundle-live-sandbox --allow-github-side-effects --github-repo neonwatty/agent-conveyor-dogfood-sandbox --branch-prefix dogfood/ --max-prs 1 --max-iterations 2 --max-runtime-minutes 30 --receipt-output /tmp/setup-bundle-live-sandbox-receipt.json --json` writes a preflight-only receipt proving the live sandbox repo, branch prefix, PR/iteration/runtime bounds, setup-bundle show requirement, and zero GitHub commands executed by preflight.
- [ ] `conveyor qa-plan ship-it-loop` and `conveyor qa-run ship-it-loop --receipt-output /tmp/ship-it-loop-receipt.json --json` prove push, PR, and merge commands fail closed before permission grants, deliver after grants, block ship-it continuation until all lifecycle receipts exist, and record conflict retry exhaustion as a blocker.
...
- [ ] `conveyor loop-templates --list --json` includes `app_visible_build_loop`, `visual_diff_loop`, `test_coverage_loop`, `pr_ci_merge_loop`, `build_then_clear`, and `compact_then_continue`.
...
- [ ] App-native loop drill: create a disposable binding with manager and worker Codex app thread ids, run bounded `dispatch --watch`, run `app-heartbeat` for each role, verify `app-loop-status --json` reports healthy leases, then age or omit one heartbeat and verify `app-wakeup-plan --json` recommends only the stale role.
- [ ] App bootstrap template drill: inspect `create-disposable-binding --json` and verify `heartbeat_recommendations` includes role-specific `app-heartbeat` prompts, direct inbox commands, `app-loop-status`, `app-wakeup-dispatch`, and sent/skipped/blocked `app-wakeup-record-delivery` command templates.
- [ ] App wake orchestration drill: run `app-wakeup-dispatch --json` for a healthy app loop and verify both roles are `skipped_healthy`; run it for a stale or missing heartbeat loop and verify stale roles are `ready_to_send` only when a Codex app thread id is present, missing Dispatch remains `dispatcher.required=true`, and telemetry contains `app_wakeup_dispatch_planned`.
- [ ] App wake adapter receipt drill: after `app-wakeup-dispatch --json`, record a `sent` outcome with `app-wakeup-record-delivery` only for a `send_ready=true` action; verify attempting `sent` for a `skipped_healthy` or `blocked_missing_thread` action fails and does not write `app_wakeup_delivery_recorded` telemetry.
- [ ] App worker rotation drill: after a saved `conveyor handoff`, run `app-worker-rotation-plan --old-worker-thread-id <active worker thread> --require-handoff --json` and verify it emits create/archive actions only for the active bound worker; run the same plan with the manager or unrelated thread id and verify it is blocked with no archive action. After app `create_thread` and exact old-thread archive, run `app-worker-rotation-record` and verify the worker session points at the new thread id with a null heartbeat.
